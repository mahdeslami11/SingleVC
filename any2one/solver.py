...
class Solver():
	def __init__(self, config):
		...
		self.scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=self.config["lr_decay"],
																last_epoch=self.config['resume_num'])
	...
	def train(self):
		...
		for epoch in range(self.init_epoch, self.total_epochs):
			...
			for step, (input_mels, input_masks, target_mels, word, overlap_lens) in enumerate(self.train_data_loader):
				...
				# loss = self.criterion(fake_mels,target_mels)
				losses = []
				...
				if total_step % self.step_record_time == 0:
					# self.logging.info('observe')
					# self.watch.observe(total_step=total_step,ag_lr=ag_lr,bg_lr=bg_lr,A_loss = A_loss,B_loss = B_loss)
...
if __name__ == '__main__':
	...
	config_path = r"/home/gyw/workspace/program/VC/SingleVC/any2one/config.yaml"
